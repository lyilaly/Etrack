var App = {
    ...
    _navDoc: function () {
        ...
        function t() {
            e.off("resize.theme.nav"),
            e.off("scroll.theme.nav"),
            n.css({ position: "", left: "", top: "" })
        }
        function i() {
            function o() {
                i.containerTop = $(".docs-content").offset().top - 40,
                i.containerRight = $(".docs-content").offset().left + $(".docs-content").width() + 45,
                t()
            }
            function t() {
                var o = e.scrollTop(),
                ...
                return t ? void n.css({ position: "fixed", left: i.containerRight, top: 40 }) : n.css({ position: "", left: "", top: "" })
            }
            var i = {};
            o(),
    ...
    _tooltips: function () {
        if ($(window).width() > 768) {
            if (App._isWithTooltips)
                return;
            App._isWithTooltips = !0,
            $('[data-toggle="tooltip"]').tooltip()
        } else {
            if (!App._isWithTooltips)
                return;
            App._isWithTooltips = !1,
